...
class CustomDataset(Dataset):
    def __init__(self, root_dir, transform=None, indices=None):
        ...
        self.indices = indices if indices is not None else list(range(len(self.image_filenames)))

    def __len__(self):
        return len(self.indices)

    def __getitem__(self, idx):
        # Use self.indices[idx] instead of idx
        image_name = self.image_filenames[self.indices[idx]]
        image_path = os.path.join(self.image_folder, image_name)
        mask_path = os.path.join(self.mask_folder, image_name.replace(".jpg", ".png").replace("image", "mask"))
